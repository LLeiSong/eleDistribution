...
Figure 7 is the combination of predicted suitability maps.
...
ggsave(file.path(fg_path, "figure7_suit_maps_90w_135h_500dpi_1.5c.png"), 
       bg = "white", width = 90, height = 135, 
       units = "mm", dpi = 500)
```

### Figure 8

Figure 8 is the landscape connectivity for long-distance migration.
...
col_blocks <- data.frame(
    ...
    name = c("Intensive farmland", "Intensive settlement", 
             "Intensive farmland & settlement", 
             "Mountain peaks"))
...
blocks_s1 <- blocks_s1 %>% 
    mutate(blocks = factor(
        blocks, levels = 1:4,
        labels = c("Intensive farmland", "Intensive settlement", 
                   "Intensive farmland & settlement", 
                   "Mountain peaks"))) %>% 
    dplyr::select(blocks)
...
legend_common <- ggplot() +
    ...
    ggnewscale::new_scale_fill() +
    geom_raster(data = data.frame(x = 1:5, y = 1:5, z = factor(1:5)),
                aes(x = x, y = y, fill = z)) +
    scale_fill_manual(
...
ggsave(file.path(fg_path, "figure8_long_connect_190w_122h_500dpi_2c.png"), 
       bg = "white", width = 190, height = 122, 
       units = "mm", dpi = 500)
```

### Figure 9

Figure 9 is the main figure for the detected corridors and example path across corridors.
...
ggplot() +
    geom_sf(data = habitat_clusters, aes(fill = "Habitat cluster"),
            color = "transparent") +
    geom_sf_text(data = habitat_clusters,
                 aes(label = cluster), color = "black", 
                 show_guide = FALSE) +
    geom_sf(data = corridors, aes(fill = "Corridor"), 
            color = "transparent") +
    scale_fill_manual(NULL, values = c(col_habitats, "gray50"),
                      breaks = c("Habitat cluster", 'Corridor'),
                      guide = guide_legend(
                          override.aes = list(linetype = "blank", 
                                              shape = NA))) +
...
ggsave(file.path(fg_path, "figure9_corridors_70w_70h_500dpi_1c.png"), 
       bg = "white", width = 70, height = 70, 
       units = "mm", dpi = 500)
```
...
Figure 10 is the environmental variable values across the random paths shown in figure 9.
...
Figure 11 shows the simulated landscape connectivity for short-distance move.
...
ggsave(file.path(fg_path, "figure11_clusters_190w_170h_500dpi_1c.png"), 
...
ggsave(file.path(fg_path, "figure_variable_response.png"), 
       width = 6.5, height = 3, dpi = 500, bg = "white")
```

### Supplementary Figure S3-S5: Response curve of all variables at coarse scale
...
### Supplementary Figure S*: Response curve of all variables at fine scale

Did not use in the manuscript.
