...
```{r}
library(rast)
source(file.path(here, "scripts/upscaling.R"))
...
crs_mer <- crs(rast(file.path(data_path, "ndvi_harmonic_1000m.tif")), proj = T)
...
execGRASS('r.in.gdal', flags = c("o", "overwrite"),
          input = file.path(data_path, "ndvi_harmonic_1000m.tif"),
          band = 1,
          output = "template")
...
# resources
ndvi <- rast(file.path(data_path, "ndvi_harmonic_1000m.tif"))
ndvi_mean <- rast(file.path(data_path, "ndvi_mean_1000m.tif"))
...
# Concatenate them
vars <- c(ndvi, wc, dsm, terrains, dists, lc)
vars <- mask(crop(vars, bry), bry)
rm(bry, ndvi, wc, dsm, terrains, dists, lc)

# Write out
writeRaster(vars, file.path(data_path, "variables_1000m.tif"))
```

##### WDPA as the regulation

WPDA is used to make the regulation of sampling efforts for MaxEnt. May not like other studies that people don't fully know if the samples are biased or not, we know that the samples of elephant occurrence are strongly biased to national parks/protected ares. Thus, reducing the sampling bias to it is very necessary for environmental suitability modeling.
...
wdpa <- read_sf(file.path(data_path, "wdpa_tanzania.geojson"))
# Remove pure forest and marine reserves
nms <- unique(wdpa$DESIG)[grepl("forest|Forest|Marine", unique(wdpa$DESIG))]
wdpa <- wdpa[!wdpa$DESIG %in% nms, ]; rm(nms)

writeVECT(wdpa, 'wdpa', v.in.ogr_flags = 'overwrite')
rm(wdpa)
execGRASS('v.to.rast', flags = c("overwrite"),
                      parameters = list(input = 'wdpa', 
                                        output = 'wdpa',
                                        use = 'val',
                                        value = 1))
execGRASS('r.grow.distance', flags = c("overwrite"),
