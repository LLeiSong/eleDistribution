ul_env_space_modeling <- function(data_path, result_path, scale) {
    ...
    message("Read environmental variables.")
    vars <- read_stars(file.path(data_path, 
                                 sprintf("ul_variables_%s.tif", scale)))
    ...
    message("Cross validation tuning.")
    ...
    tuning_cv <- do.call(rbind, pblapply(1:nrow(params), function(n) {
    ...
    save(tuning_cv, 
         file = file.path(result_path, sprintf("tuning_cv_%s.rda", scale)))
    ...
    best_params <- rbind(tuning_cv %>% filter(auc == max(auc)),
        ...
        unique()
    message(sprintf("%s sets of parameters to run.", nrow(best_params)))
    ...
    save(runs, file = file.path(result_path, sprintf("runs_%s.rda", scale)))
}